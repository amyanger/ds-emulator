...
// compute_halfword_address() and consumed by dispatch_halfword. `rn` is
// kept so the wrapper can suppress the writeback when Rn == Rd on a load
// (see §5.5 of the slice 3b3 design).
...
// the variant-specific behavior lives in execute_halfword_transfer_core.
...
u32 execute_halfword_transfer_core(Arm7State& state,
                                   Arm7Bus& bus,
                                   u32 address,
                                   u32 rd,
                                   HalfwordTransferKind kind,
                                   u32 instr_addr) {
    if (kind == HalfwordTransferKind::STRH) {
        // Rd == R15 on a halfword store reads as PC+12, matching the
        // word-STR pipeline convention (see arm7_loadstore.cpp).
        ...
        bus.write16(address & ~1u, static_cast<u16>(rd_value & 0xFFFFu));
        return 1;
    }

    u32 value;
    switch (kind) {
    case HalfwordTransferKind::LDRH: {
        ...
        value = static_cast<u32>(bus.read16(address & ~1u));
        break;
    }
    case HalfwordTransferKind::LDSB: {
        // Byte loads have no alignment concerns — there's only one byte at
        // any address. Sign-extend bit 7 via the standard u8 -> i8 -> i32 -> u32
        // promotion chain.
        const u8 raw = bus.read8(address);
        value = static_cast<u32>(static_cast<i32>(static_cast<i8>(raw)));
        break;
    }
    case HalfwordTransferKind::LDSH: {
        ...
        value = static_cast<u32>(static_cast<i32>(static_cast<i16>(raw)));
        break;
    }
    case HalfwordTransferKind::STRH:
        // Unreachable — handled above.
        value = 0;
        break;
    }

    // Rd==R15 as a halfword load destination is UNPREDICTABLE on ARMv4T.
    // Match the pre-refactor behavior: log a warn, then use the standard
    // write_rd helper which masks PC to word alignment and leaves CPSR.T
    // unchanged.
    if (rd == 15) {
        DS_LOG_WARN("arm7: halfword load with Rd == 15 (unpredictable) at 0x%08X", state.pc);
    }
...
u32 dispatch_halfword(Arm7State& state, Arm7Bus& bus, u32 instr, u32 instr_addr) {
    ...
    if (!l) {
        ...
        switch (sh) {
        case 0:
            DS_LOG_WARN("arm7: SWP encoding encountered, not yet supported at 0x%08X", state.pc);
            break;
        case 1: {
            const HalfwordAddress addr = compute_halfword_address(state, instr);
            const u32 rd = (instr >> 12) & 0xFu;
            execute_halfword_transfer_core(
                state, bus, addr.address, rd, HalfwordTransferKind::STRH, instr_addr);
            // Stores always update Rn when writeback is enabled — no
            // Rn==Rd suppression (that rule is load-only per §5.5 of the
            // slice 3b3 design spec).
            if (addr.writeback) {
        ...
        case 1: {
            ...
            const u32 rd = (instr >> 12) & 0xFu;
            execute_halfword_transfer_core(
                state, bus, addr.address, rd, HalfwordTransferKind::LDRH, instr_addr);
            // Loads suppress Rn writeback when Rn == Rd so the loaded
            // value wins (see §5.5 of the slice 3b3 design spec).
        ...
        case 2: {
            ...
            const u32 rd = (instr >> 12) & 0xFu;
            execute_halfword_transfer_core(
                state, bus, addr.address, rd, HalfwordTransferKind::LDSB, instr_addr);
            if (addr.writeback && addr.rn != rd) {
                state.r[addr.rn] = addr.writeback_rn;
            }
            break;
        ...
        case 3: {
            ...
            const u32 rd = (instr >> 12) & 0xFu;
            execute_halfword_transfer_core(
                state, bus, addr.address, rd, HalfwordTransferKind::LDSH, instr_addr);
            if (addr.writeback && addr.rn != rd) {
                state.r[addr.rn] = addr.writeback_rn;
            }
            break;
